...
class SDN_stat_payload : public payload {
        ...
        // pair -> <nid, linkweight>
        vector< pair<unsigned int , unsigned int> > neighbor_info;
    ...
    public:
        ...
        // get the neighbor_info vector
        vector< pair<unsigned int , unsigned int> >& Get_neighbor_info() { return neighbor_info; }

        // set the information
        void Set_neighbor_info(vector< pair<unsigned int , unsigned int> > _neighbor_info) {
            neighbor_info.assign(_neighbor_info.begin(), _neighbor_info.end());
        }
...
// controller
unsigned int Controller_id = node::getNodeNum();
...
class SDN_switch: public node {
        ...
        map<unsigned int, unsigned int> path;
...
class SDN_stat_pkt_gen_event: public event {
    ...
    protected:
        SDN_stat_pkt_gen_event (unsigned int _trigger_time, void *data): event(_trigger_time), src(BROCAST_ID), dst(BROCAST_ID){
            ...
            dst = data_ptr->dst_id;
            // pkt = data_ptr->_pkt;
            neighbor_info.assign(data_ptr->neighbor_info_pb.begin(), data_ptr->neighbor_info_pb.end());
            //nid = data_ptr->nid_id;
            //lweight = data_ptr->lweight_id;
            msg = data_ptr->msg;
        }
    ...
    public:
        ...
        class pkt_gen_data{
            public:
                unsigned int src_id; // the controller
                unsigned int dst_id; // the node that should update its rule
                vector< pair<unsigned int, unsigned int> > neighbor_info_pb;
                //unsigned int nid_id;
                //unsigned int lweight_id;
                string msg;
                // packet *_pkt;
        };
...
void SDN_stat_pkt_gen_event::trigger() {
    ...
    // payload
    pld->setMsg(msg);
    pld->Set_neighbor_info(neighbor_info);
...
unsigned int SDN_stat_pkt_gen_event::event_priority() const {
    string string_for_hash;
    string_for_hash = to_string(getTriggerTime()) + to_string(src) + to_string(dst) + to_string(neighbor_info[0].first); //to_string (pkt->getPacketID());
    return get_hash_value(string_for_hash);
}
...
// the ctrl_stat_event function is used to tell switch it can send the link weight to the control
void stat_packet_event (unsigned int con_id, unsigned int id, vector<pair<unsigned int, unsigned int>> neigh_info, unsigned int t = event::getCurTime(), string msg = "default"){
// void ctrl_stat_event (unsigned int dst, unsigned int t = 0, string msg = "default"){
    if ( id == BROCAST_ID || node::id_to_node(id) == nullptr ) {
        cerr << "id is incorrect" << endl; return;
    }
    ...
    e_data.neighbor_info_pb.assign(neigh_info.begin(), neigh_info.end());
    e_data.msg = msg;
...
void SDN_switch::recv_handler (packet *p){
    ...
    unsigned int dstId = p->getHeader()->getDstID();
    unsigned int con_id = p->getHeader()->getSrcID();
    //cout << "destid: " << dstId << endl;
    map<unsigned int, unsigned int>::iterator next;
    next = path.find(dstId);
    if (p->type() == "SDN_data_packet" && next != path.end()) { // the switch receives a packet from the other switch
        ...
        p2->getHeader()->setNexID ( next -> second );
        p2->getHeader()->setDstID ( dstId );
        send_handler (p2);
    }
    //             p->getHeader()->setSrcID() or getSrcID()
    //             p->getHeader()->setDstID() or getDstID()
    //             p->getHeader()->setPreID() or getPreID()
    //             p->getHeader()->setNexID() or getNexID()
        ...
        // get the actID, MatId
        unsigned MatId = l3 -> getMatID();
        unsigned ActId = l3 -> getActID(); // Next hop
 ...
 // SDN_controller
    class SDN_controller: public node {
           // map<unsigned int,bool> one_hop_neighbors; // you can use this variable to record the node's 1-hop neighbors
            ...
            // for overview
            int num_destination, num_node;
            int destination[1000];

            // for ctrl_pkt
            map<unsigned int , map<unsigned int, int>> Setup;
            map<unsigned int , map<unsigned int, int>> update; // <MatID , <the node that controller already set up the path(ActID), status>>

            class updateinfo{
                public:
                    unsigned int dst, mat, act;
            };

            vector<updateinfo> updatebuffer;

            // stat_pkt
            int num_stat_new;
            int num_stat_old;
            bool stat_new, stat_old;

            // the infomation for counting routing path
            class Neighborhood_old{
            ...
            class NODE{
                ...
                public:
                    vector<Neighborhood_old> neighbor_old;
                    vector<Neighborhood_new> neighbor_new;
        ...
        protected:
            SDN_controller() : stat_new(false), stat_old(false), num_stat_new(0), num_stat_old(0){} // it should not be used
            SDN_controller(SDN_controller&) {} // it should not be used
            SDN_controller(unsigned int _id): node(_id), stat_new(false), stat_old(false), num_stat_new(0), num_stat_old(0){} // this constructor cannot be directly called by users

        public:

            // record the node that doesn't need to update
            void Record_need_not_to_update(unsigned int mat, unsigned int id){
                update[mat][id] = pathsetUp;
            }

            //  record the path
            void pathset_setup(unsigned int src, unsigned int mat){ Setup[mat][src] = notyet; }
            void pathset_update(unsigned int src, unsigned int mat){ update[mat][src] = notyet; }

            // set overveiw
            void setOverveiw(int _num_node, int _num_destination){
                num_node = _num_node;
                num_destination = _num_destination;
            }
            void setdestination(int i,int _destination){
                destination[i] = _destination;
            }
...
void SDN_controller::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_ctrl_packet") {
        ...
        // get the actID, MatId
        unsigned int MatId = l3 -> getMatID();
        unsigned int ActId = l3 -> getActID(); // Next hop

        string msg = l3 -> getMsg();    // get msg -> update or setup
        int rule = 0;
        if( msg == "setup"){
            rule = Set;
        }
        ...
        SDN_ctrl_packet *p_waiting = nullptr;
        SDN_ctrl_payload *l_waiting = nullptr;


        vector<packet*>::iterator iter;

        if(rule == Set){  // setup
            if(Setup[MatId][p->getHeader()->getPreID()] == formController){  // receive the psg from switches
                Setup[MatId][p->getHeader()->getPreID()] = pathsetUp;
            }
            else if(Setup[MatId][dstId] == notyet){
                // send pkt to switches
                p3->getHeader()->setNexID( dstId );
                send_handler(p3);
                // record the status
                Setup[MatId][dstId] = formController;
            }
        }
        else if(rule == Update){   // update
           if(update[MatId][p->getHeader()->getPreID()] == formController){  // receive the psg from switches

                update[MatId][p->getHeader()->getPreID()] = pathsetUp;

                //  find another node that can build the path
                for(int i = 0;i < updatebuffer.size();i++){
                    if(update[updatebuffer[i].mat][updatebuffer[i].act] == pathsetUp){

                        // send pkt to switches


                        ctrl_packet_event(con_id, updatebuffer[i].dst, updatebuffer[i].mat, updatebuffer[i].act, event::getCurTime(), "update");

                        // erase the waiting packet from the buffer
                        updatebuffer.erase(updatebuffer.begin()+i);
                    }
                }
            }
            else if( (update[MatId][dstId] == notyet && update[MatId][ActId] == pathsetUp) || (update[MatId][dstId] == notyet && MatId == ActId) ){  // 1.the parent node has been set the path, 2.the next hop is dst ->build the path
                // send pkt to switches
                p3->getHeader()->setNexID( dstId );
                send_handler(p3);
                //  record the status
                update[MatId][dstId] = formController;

                //cout << "dstid: " << dstId << " " << "send to switch\n";
                //cout << "The actid is: " << ActId << endl;
                //cout << "pre-node status: " << update[MatId][ActId] << endl;

            }
            else{   // pre-node hasn't build yet, put it into wait buffer
                class updateinfo data;
                data.dst = dstId;
                data.mat = MatId;
                data.act = ActId;
                updatebuffer.push_back(data);
            }
        }
    }
    else if(p->type() == "SDN_stat_packet"){

        unsigned int con_id = p->getHeader()->getDstID();
        //cout << "destid: " << dstId << endl;
        unsigned int switch_id = p->getHeader()->getSrcID();
        ...
        // get the pkt from switches, and store the info into the table
        if(msg == "setup"){
            vector<pair<unsigned int, unsigned int>>::iterator iter;
            Neighborhood_old temp;
            for(iter = (l3->Get_neighbor_info()).begin();iter != (l3->Get_neighbor_info()).end();iter++){
                temp.neighborId = iter->first;
                temp.old_weight = iter->second;
                Node[switch_id].neighbor_old.push_back(temp);
            }
            num_stat_old++;
        }
        else if(msg == "update"){
            vector<pair<unsigned int, unsigned int>>::iterator iter;
            Neighborhood_new temp;
            for(iter = (l3->Get_neighbor_info()).begin();iter != (l3->Get_neighbor_info()).end();iter++){
                temp.neighborId = iter->first;
                temp.new_weight = iter->second;
                Node[switch_id].neighbor_new.push_back(temp);
            }
            num_stat_new++;
        }

        // controller receive all the pkt from switches, can count the old path
        if(stat_old == false && num_stat_old == num_node){

            //cout << "counting old path...\n";
            ...
            for(int i = 0;i < num_destination;i++){
                ...
                for(int k = 0;k < num_node;k++){
                    ...
                    int neighborsize = Node[NowNode].neighbor_old.size();
                    //cout << "neighborsize: " << neighborsize << endl;
                    for(int t = 0;t < neighborsize;t++){

                        if( !visited[Node[NowNode].neighbor_old[t].neighborId] && (distance[NowNode]+Node[NowNode].neighbor_old[t].old_weight) <= distance[Node[NowNode].neighbor_old[t].neighborId] ){  // if the new path is shorter than old one, change the record
                            ...
                            iter = Node[Node[NowNode].neighbor_old[t].neighborId].OldPath.find(destination[i]);
                            if((distance[NowNode]+Node[NowNode].neighbor_old[t].old_weight) < distance[Node[NowNode].neighbor_old[t].neighborId]){
                                Node[Node[NowNode].neighbor_old[t].neighborId].OldPath[destination[i]] = NowNode;
                            }
                            else if( iter != Node[Node[NowNode].neighbor_old[t].neighborId].OldPath.end() && (unsigned int)NowNode < iter->second){
                                Node[Node[NowNode].neighbor_old[t].neighborId].OldPath[destination[i]] = NowNode;
                            }
                            distance[Node[NowNode].neighbor_old[t].neighborId] = distance[NowNode]+Node[NowNode].neighbor_old[t].old_weight;
            ...
            for(int i = 0;i < num_node;i++){
                for(iter = Node[i].OldPath.begin();iter != Node[i].OldPath.end();++iter){
                    pathset_setup( i, iter->first);
                    //cout << "old path, matid: " << iter->first << ", actid: " << iter->second << endl;
                    ctrl_packet_event(con_id, i, iter->first, iter->second, event::getCurTime(), "setup");
                }
            }

        }
        else if(stat_new == false && num_stat_new == num_node){  // controller receive all the pkt from switches, can count the new path
            ...
            for(int i = 0;i < num_destination;i++){
                ...
                for(int k = 0;k < num_node;k++){
                    ...
                    visited[NowNode] = 1;  //the node has been visited already
                    int neighborsize = Node[NowNode].neighbor_new.size();
                    for(int t = 0;t < neighborsize;t++){

                        if( !visited[Node[NowNode].neighbor_new[t].neighborId] && (distance[NowNode]+Node[NowNode].neighbor_new[t].new_weight) <= distance[Node[NowNode].neighbor_new[t].neighborId] ){  // if the new path is shorter than old one, change the record

                            // change record
                            // compare the old record, if the distance is the same, but the number of destination of node if smaller than the old one, change it
                            map<unsigned int, unsigned int>:: iterator iter;
                            iter = Node[Node[NowNode].neighbor_new[t].neighborId].NewPath.find(destination[i]);
                            if((distance[NowNode]+Node[NowNode].neighbor_new[t].new_weight) < distance[Node[NowNode].neighbor_new[t].neighborId]){
                                Node[Node[NowNode].neighbor_new[t].neighborId].NewPath[destination[i]] = NowNode;
                            }
                            else if( iter != Node[Node[NowNode].neighbor_new[t].neighborId].NewPath.end() && (unsigned int)NowNode < iter->second){
                            Node[Node[NowNode].neighbor_new[t].neighborId].NewPath[destination[i]] = NowNode;
                            }
                            distance[Node[NowNode].neighbor_new[t].neighborId] = distance[NowNode]+Node[NowNode].neighbor_new[t].new_weight;
            ...
            // clean the path that doesn't to change
            ...
            for(int i = 0;i < num_node;i++){
                for(iter = Node[i].OldPath.begin();iter != Node[i].OldPath.end();++iter){
                    ...
                    if(same->second == iter->second){  // find it, and delete
                        // set the update path
                        Record_need_not_to_update(iter->first, i);
            ...
            for(int i = 0;i < num_node;i++){
                for(iter = Node[i].NewPath.begin();iter != Node[i].NewPath.end();++iter){
                    //string msg = to_string(iter->first) + " " + to_string(iter->second);
                    pathset_update( i, iter->first);
                    //cout << "node:" << i << endl;
                    //cout << "update: " << iter-> first << " " << iter->second << endl;
                    ctrl_packet_event(con_id, i, iter->first, iter->second, event::getCurTime(), "update");
                }
            }
...
int main()
{
   ...
   // read the input and generate switch nodes
    ...
    int InsTime, UdpTime, SimDuration;
    cin >> InsTime >> UdpTime >> SimDuration;
    ...
    SDN_controller *p_con = nullptr;
    p_con = dynamic_cast<SDN_controller*> (node::id_to_node(con_id));

    // set overview
    p_con -> setOverveiw(num_node, num_destination);
    for(int i = 0;i < num_destination;i++){
        p_con -> setdestination(i,destination[i]);
    }

    // get the node
    int linkid = 0, node1 = 0, node2 = 0, oldW = 0, newW = 0;

    pair<unsigned int, unsigned int> temp_node1, temp_node2; //<nid, lweight>
    vector <pair<unsigned int, unsigned int>> stat_infoNew[1000], stat_infoOld[1000];

    for(int i = 0;i < links;i++){

        cin >> linkid >> node1 >> node2 >> oldW >> newW;

        // store their neighbor and set switches' neighbors

        temp_node1.first = node2;
        temp_node1.second = oldW;
        stat_infoOld[node1].push_back(temp_node1);
        temp_node1.second = newW;
        stat_infoNew[node1].push_back(temp_node1); // store node1's neighbor_info
        temp_node2.first = node1;
        temp_node2.second = oldW;
        stat_infoOld[node2].push_back(temp_node2);
        temp_node2.second = newW;
        stat_infoNew[node2].push_back(temp_node2); // store node2's neighbor_info
        node::id_to_node(node1)->add_phy_neighbor(node2);
        node::id_to_node(node2)->add_phy_neighbor(node1);

        // connect nodes to their SDN_controller
    ...
    int Counter = 0;
    // read the input
    while(cin >> Data[Counter].t >> Data[Counter].src >> Data[Counter].dst){
        Counter++;
    }

    for(int i = 0;i < num_node;i++){
        stat_packet_event(con_id, i, stat_infoOld[i], InsTime, "setup");
        stat_packet_event(con_id, i, stat_infoNew[i], UdpTime, "update");
    }

    map<unsigned int, unsigned int>::iterator iter,same;
    //use data_packet_event to add an initial event

    for(int i = 0;i < Counter;i++){
        data_packet_event(Data[i].src, Data[i].dst, Data[i].t);
    }
