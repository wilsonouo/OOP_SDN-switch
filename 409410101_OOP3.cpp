 ...
 // SDN_controller
    class SDN_controller: public node {
           // map<unsigned int,bool> one_hop_neighbors; // you can use this variable to record the node's 1-hop neighbors
            ...
            //vector<packet*> updateBuffer;   // for update
...
bool compare(data_event x, data_event y){
   ...
   // read the input and generate switch nodes
    ...
    for(int i = 0;i < num_node;i++){
        for(iter = Node[i].OldPath.begin();iter != Node[i].OldPath.end();++iter){
            ...
            p_con -> pathset_setup( i, iter->first);
            ctrl_packet_event(con_id, i, iter->first, iter->second, InsTime, "setup");
            ...
            if(same->second == iter->second){  // find it, and delete
                // set the update path
                p_con -> Record_need_not_to_update(iter->first, i);
    ...
    for(int i = 0;i < num_node;i++){
        for(iter = Node[i].NewPath.begin();iter != Node[i].NewPath.end();++iter){
            //string msg = to_string(iter->first) + " " + to_string(iter->second);
            p_con -> pathset_update( i, iter->first);
            //cout << "node:" << i << endl;
            //cout << "update: " << iter-> first << " " << iter->second << endl;
            ctrl_packet_event(con_id, i, iter->first, iter->second, UdpTime, "update");
        }
    }
...
/*
                for(int i = 0;i < updateBuffer.size();i++){
                    cout << "hi" << endl;
                    p_waiting = dynamic_cast<SDN_ctrl_packet*>(updateBuffer[i]);
                    l_waiting = dynamic_cast<SDN_ctrl_payload*> (p_waiting->getPayload());
                    cout << l_waiting->getMatID() << endl;
                    if(update[l_waiting->getMatID()][l_waiting->getActID()] == pathsetUp){

                        // send pkt to switches
                        cout << updateBuffer[i]->getHeader()->getDstID() << endl;
                        p_waiting->getHeader()->setNexID( updateBuffer[i]->getHeader()->getDstID() );
                        send_handler(p_waiting);

                        //  record the status
                        update[l_waiting->getMatID()][updateBuffer[i]->getHeader()->getDstID()] = formController;
