...
void SDN_switch::recv_handler (packet *p){
    ...
    //             p->getHeader()->setNexID() or getNexID()
        ...
        string str_destid, str_nextid;
        int middle = msg.find(" ",0);
        int length = (msg.length()-1) - middle;
        str_destid = str_destid.assign(msg,0,middle);
        //cout << "node: " << getNodeID() << endl;
        //cout << "str_destid: " << str_destid << endl;
        str_nextid = str_nextid.assign(msg,middle+1,length);
        //cout << "nextid: " << str_nextid << endl;
        unsigned int destid, nextid;
        destid = stoul(str_destid, nullptr, 0);
        //cout << "num_str_destid: " << destid << endl;
        nextid = stoul(str_nextid, nullptr, 0);
        //cout << "num_nextid: " << nextid << endl;
...
int main()
{
    ...
    for(int i = 0;i < Counter;i++){
        ...
        if(Data[i].t >= UdpTime && update == false){
            // update the path
            for(int i = 0;i < num_node;i++){
                for(iter = Node[i].NewPath.begin();iter != Node[i].NewPath.end();++iter){
                    string msg = to_string(iter->first) + " " + to_string(iter->second);
                    ctrl_packet_event(i, UdpTime, msg);
                }
            }
            update = true;
        }
